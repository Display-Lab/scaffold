...
class Trend_R(Signal):
    ...
    def detect(
        ...
        if Trend_R.check(perf_data) is False:
            return []

        if not Trend_R.last_three_periods_are_valid_and_consecutive(perf_data):
            return []

        slope = Trend_R._detect(perf_data)

        if not slope:
            return []

        return [Trend_R._resource(slope)]
