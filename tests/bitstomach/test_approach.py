...
from scaffold.bitstomach.signals import Approach
...
def perf_data() -> pd.DataFrame:
    performance_data = [
        ...
        [True, 157, "BP01", "2022-08-01", 0.75, 100.0],
...
def comparator_data() -> pd.DataFrame:
    comparator_data = [
        ...
        ["BP01", "2022-10-01", 0.90, "http://purl.obolibrary.org/obo/PSDO_0000128"],
        ...
        ["BP02", "2022-10-01", 0.15, "http://purl.obolibrary.org/obo/PSDO_0000128"],
...
def test_approach_is_rdf_type():
    g: Graph = Graph()
    mi = g.resource(BNode())
    mi.add(RDF.type, PSDO.approach_content)

    assert Approach.is_rdf_type_of(mi)


def test_disposition():
    g: Graph = Graph()

    # Hand create an Approach with dispositions
    ...
    dispositions = Approach.disposition(mi)
    assert len(dispositions)
    assert g.resource(PSDO.approach_content) in dispositions


def test_detect_handles_empty_datframe():
    with pytest.raises(ValueError):
        Approach.detect(pd.DataFrame(), pd.DataFrame())
...
def test_signal_properties(perf_data, comparator_data):
    ...
    assert slope == pytest.approx(0.07)
...
perf_level_test_set = [
    ...
    (
        ...
        "approach no peer_average_comparator",
    ),
    (
...
def test_moderators():
    ...
    r.add(RDF.type, PSDO.approach_content)

    # add approach properites
    ...
    moderators = Approach.moderators([r])
